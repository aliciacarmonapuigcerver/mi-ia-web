...
@st.cache_resource
def load_my_model():
    return tf.keras.models.load_model("model_omnist.keras")
...
if canvas_result.image_data is not None:
    ...
    if confianza < 0.80:
        st.warning(f"Confianza baja ({confianza:.2f}). ¿Podrías dibujar más claro?")
    else:
        st.success(f"Confianza alta ({confianza:.2f})")
